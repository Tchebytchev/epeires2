...
use Agenda\Entity\Report;
use Zend\View\Model\ViewModel;
use Zend\Form\Annotation\AnnotationBuilder;
use DoctrineModule\Stdlib\Hydrator\DoctrineObject;
use Zend\View\Model\JsonModel;
use Agenda\Entity\Element;
...
class ReportController extends AbstractActionController
{
    ...
    public function exportAction()
    {
        ...
        if ($this->zfcUserAuthentication()->hasIdentity() && $id !== null) {
            ...
            $tbs->LoadTemplate('data/templates/cr_agenda_model_v1.odt', OPENTBS_ALREADY_UTF8);
            ...
            $report = $em->getRepository('Agenda\Entity\Report')->find($id);
            ...
            foreach ($em->getRepository('Agenda\Entity\ReportCategory')->findAll() as $cat) {
                $catevents = array();
            ...
            // send the file
            $tbs->Show(OPENTBS_DOWNLOAD, 'rapport_Agenda_semaine' . $report->getWeek() . '_' . $report->getYear() . '.odt');
        } else {}
    }
    ...
    private function getFormReport($id)
    {
        ...
        if ($id) {
            $report = $objectManager->getRepository('Agenda\Entity\Report')->find($id);
            if ($report) {
                $form->bind($report);
    ...
    public function showAction()
    {
        if ($this->zfcUserAuthentication()->hasIdentity()) {
            ...
            if ($id !== null) {
                ...
                foreach ($em->getRepository('Agenda\Entity\ReportCategory')->findBy(array(), array(
                    'place' => 'ASC'
                )) as $reportcategory) {
    ...
    public function affectcategoryAction()
    {
        ...
        if ($id !== null && $reportid !== null) {
            ...
            $report = $em->getRepository('Agenda\Entity\Report')->find($reportid);
            $element = null;
            ...
            if ($catid !== null) {
                $cat = $em->getRepository('Agenda\Entity\ReportCategory')->find($catid);
